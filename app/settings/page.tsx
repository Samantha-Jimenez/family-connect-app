...
const Settings = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const firstName = formData.get('floating_first_name') as string;
    const lastName = formData.get('floating_last_name') as string;
    const fullName = `${firstName} ${lastName}`;
    const birthday = formData.get('floating_birthday') as string;
    
    saveUserToDB(fullName, username, birthday);
  };

  return (
    <AuthGuard>
      <form className="card max-w-md bg-base-100 shadow-xl p-6 mx-auto mt-24" onSubmit={handleSubmit}>
        ...
        <div className="relative z-0 w-full mb-5 group">
            <input type="username" value={user?.signInDetails?.loginId || username} onChange={(e) => setUsername(e.target.value)} name="floating_username" id="floating_username" className="block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
      ...
      <form className="card max-w-md bg-base-100 shadow-xl p-6 mx-auto mt-24" onSubmit={handleSubmit}>
        ...
        <div className="grid md:grid-cols-2 md:gap-6">
            <div className="relative z-0 w-full mb-5 group">
                <input type="date" name="floating_birthday" id="floating_birthday" className="block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
        ...
        {/* <div className="relative z-0 w-full mb-5 group">
            <input type="color" name="floating_color" id="floating_color" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            <label htmlFor="floating_color" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">FavoriteColor</label>
        </div> */}
        <div className="relative z-0 w-full mb-5 group">
